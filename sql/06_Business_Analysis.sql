...
/* 3. Category profitability — find high-revenue / low-profit categories */
SELECT
    category,
    SUM(gross_revenue) AS gross_revenue,
    SUM(discount_value) AS discount_value,
    ...
    SUM(gross_profit) AS gross_profit,
    CAST(SUM(gross_profit) / NULLIF(SUM(sales_after_discount - refund_value),0) AS DECIMAL(10,4)) AS profit_margin,
    CAST(SUM(is_returned) * 1.0 / NULLIF(COUNT(DISTINCT order_id),0) AS DECIMAL(10,4)) AS return_rate
FROM analytics.vw_OrderProfitability
GROUP BY category
...
SELECT
    CASE WHEN order_count = 1 THEN 'New / One-time' ELSE 'Repeat' END AS customer_type,
    COUNT(*) AS customers,
    SUM(net_revenue) AS net_revenue,
    SUM(gross_profit) AS gross_profit,
    CAST(SUM(gross_profit) / NULLIF(SUM(net_revenue),0) AS DECIMAL(10,4)) AS profit_margin
FROM CustomerOrders
GROUP BY CASE WHEN order_count = 1 THEN 'New / One-time' ELSE 'Repeat' END;
