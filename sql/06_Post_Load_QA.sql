...
SELECT TOP (20)
    order_id,product_id,gross_revenue,discount_value,sales_after_discount,refund_amount,
    net_revenue,product_cost,shipping_cost,return_cost,gross_profit
FROM analytics.vw_OrderProfitability
...
FROM analytics.vw_OrderProfitability
WHERE is_delivered=1 AND net_revenue<0;
...
FROM analytics.vw_OrderProfitability
WHERE product_cost<0 OR shipping_cost<0 OR return_cost<0 OR refund_amount<0;
GO
...
SELECT COUNT(*) AS delivered_date_with_non_delivered_status
FROM analytics.vw_OrderProfitability
...
SELECT
    SUM(gross_revenue-discount_value-sales_after_discount) AS sales_identity_difference,
