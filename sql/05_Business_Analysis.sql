...
SELECT COUNT(DISTINCT CASE WHEN order_status='Delivered' THEN order_id END) AS delivered_orders,
       ...
       SUM(CASE WHEN order_status='Delivered' THEN product_cost ELSE 0 END) AS product_cost,
       SUM(CASE WHEN order_status='Delivered' THEN shipping_cost+return_cost ELSE 0 END) AS operational_cost,
...
SELECT TOP (50) product_id,product_name,category,subcategory,brand,product_tier,
       COUNT(DISTINCT CASE WHEN order_status='Delivered' THEN order_id END) AS delivered_orders,
       SUM(CASE WHEN order_status='Delivered' THEN net_revenue ELSE 0 END) AS net_revenue,
       SUM(CASE WHEN order_status='Delivered' THEN gross_profit ELSE 0 END) AS gross_profit,
       CAST(100.0*SUM(CASE WHEN order_status='Delivered' THEN gross_profit ELSE 0 END)/NULLIF(SUM(CASE WHEN order_status='Delivered' THEN net_revenue ELSE 0 END),0) AS DECIMAL(10,2)) AS margin_pct
...
SELECT CASE WHEN discount_pct<0.05 THEN '<5%' WHEN discount_pct<0.10 THEN '5-10%'
            WHEN discount_pct<0.15 THEN '10-15%' WHEN discount_pct<0.20 THEN '15-20%' ELSE '20%+' END AS discount_band,
       COUNT(DISTINCT CASE WHEN order_status='Delivered' THEN order_id END) AS delivered_orders,
       SUM(CASE WHEN order_status='Delivered' THEN discount_value ELSE 0 END) AS discount_value,
       SUM(CASE WHEN order_status='Delivered' THEN net_revenue ELSE 0 END) AS net_revenue,
       SUM(CASE WHEN order_status='Delivered' THEN gross_profit ELSE 0 END) AS gross_profit,
       CAST(100.0*SUM(CASE WHEN order_status='Delivered' THEN gross_profit ELSE 0 END)/NULLIF(SUM(CASE WHEN order_status='Delivered' THEN net_revenue ELSE 0 END),0) AS DECIMAL(10,2)) AS margin_pct
FROM analytics.vw_OrderProfitability
GROUP BY CASE WHEN discount_pct<0.05 THEN '<5%' WHEN discount_pct<0.10 THEN '5-10%'
             WHEN discount_pct<0.15 THEN '10-15%' WHEN discount_pct<0.20 THEN '15-20%' ELSE '20%+' END
ORDER BY MIN(discount_pct);
GO

-- 6. Return reasons and total leakage
SELECT return_reason,COUNT(*) AS approved_returns,SUM(refund_amount) AS refund_value,SUM(total_return_cost) AS total_return_cost
FROM analytics.vw_ReturnsOperations
WHERE return_status='Approved'
GROUP BY return_reason ORDER BY total_return_cost DESC;
GO

-- 7. Delivery performance versus returns
SELECT delivery_status_clean AS delivery_group,
       ...
       CAST(100.0*COUNT(DISTINCT CASE WHEN is_returned=1 AND is_delivered=1 THEN order_id END)/NULLIF(COUNT(DISTINCT CASE WHEN is_delivered=1 THEN order_id END),0) AS DECIMAL(10,2)) AS return_rate_pct
FROM analytics.vw_OrderProfitability
...
SELECT region,COUNT(DISTINCT CASE WHEN order_status='Delivered' THEN order_id END) AS orders,
       SUM(CASE WHEN order_status='Delivered' THEN net_revenue ELSE 0 END) AS net_revenue,
       SUM(CASE WHEN order_status='Delivered' THEN gross_profit ELSE 0 END) AS gross_profit,
       CAST(100.0*SUM(CASE WHEN order_status='Delivered' THEN gross_profit ELSE 0 END)/NULLIF(SUM(CASE WHEN order_status='Delivered' THEN net_revenue ELSE 0 END),0) AS DECIMAL(10,2)) AS margin_pct
FROM analytics.vw_OrderProfitability GROUP BY region ORDER BY gross_profit DESC;

SELECT acquisition_channel,COUNT(DISTINCT customer_id) AS customers,
       SUM(CASE WHEN order_status='Delivered' THEN net_revenue ELSE 0 END) AS net_revenue,
       SUM(CASE WHEN order_status='Delivered' THEN gross_profit ELSE 0 END) AS gross_profit
FROM analytics.vw_OrderProfitability GROUP BY acquisition_channel ORDER BY gross_profit DESC;
GO

-- 9. One-time versus repeat customer economics
SELECT customer_type,COUNT(*) AS customers,AVG(delivered_orders*1.0) AS avg_orders,
       AVG(net_revenue) AS avg_net_revenue,AVG(gross_profit) AS avg_gross_profit
FROM analytics.vw_CustomerProfitability GROUP BY customer_type;
GO

-- 10. Customer value leaderboard
SELECT TOP (25) customer_id,customer_name,customer_segment,region,acquisition_channel,
       delivered_orders,net_revenue,gross_profit,customer_type
FROM analytics.vw_CustomerProfitability ORDER BY gross_profit DESC;
GO
